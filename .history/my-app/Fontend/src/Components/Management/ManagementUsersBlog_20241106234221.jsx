...
function ManagementUsersBlog() {
  ...
  const handleApprove = async (blog) => {
    try {
      // Gửi POST request với dữ liệu của blog được duyệt
      await axios.post("http://localhost:8080/api/v1/blogUser/", blog);
      alert('Cập nhật blog thành công !!')

      // Sau khi duyệt, xóa blog khỏi danh sách
      setUserBlog((prevBlogs) =>
        prevBlogs.filter((item) => item._id !== blog._id)
      );
    } catch (error) {
      ...
      <div className="courseMain">
        {userBlog.map((items) => (
          <div className="boxCourse" id={items._id}>
            ...
            <button onClick={() => handleApprove(items._id)}>Duyệt</button>
