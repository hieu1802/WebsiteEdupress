...
function CourseForm({ onClose }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:8080/api/v1/courses/', formData); // Cập nhật URL cho API của bạn
  ...
  return (
    <form onSubmit={handleSubmit} style={{ background: '#f9f9f9', padding: '20px', borderRadius: '8px' }}>
      <label>Ảnh: <input type="text" name="img" value={formData.img} onChange={handleChange} /></label>
      ...
      <label>Buổi học: <input type="number" name="lesson" value={formData.lesson} onChange={handleChange} /></label>
